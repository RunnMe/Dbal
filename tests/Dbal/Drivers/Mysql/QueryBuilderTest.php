...
namespace Running\tests\Dbal\Drivers\Mysql\QueryBuilder;

use Running\Dbal\Drivers\Mysql\Driver;
use Running\Dbal\Drivers\Mysql\QueryBuilder;
...
class QueryBuilderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testMakeSelectQuery()
    {
        ...
        $this->assertEquals(
            "SELECT t1.`a1`, j1.`a2`\nFROM `test1` AS t1, `test2` AS t2\nINNER JOIN `foo` AS j1 ON j1.id=t1.id\nLEFT JOIN `bar` AS j2 ON j2.id<t2.id\nRIGHT JOIN `baz` AS `bzz` ON j3.id>:baz\nINNER JOIN `bla` AS `b` ON j4.id=j3.id",
            $builder->makeQueryString($query)
        );

        $query = new Query();
        $query = $query->select()->from('one');
    ...
    public function testMakeUpdateQuery()
    {
        ...
        $query = new Query();
        $query = $query->update('test')->values(['foo' => ':foo', 'bar' => ':bar'])->where('id=123')->order('id DESC')
            ->limit(1);
        $this->assertEquals(
            "UPDATE `test`\nSET `foo`=:foo, `bar`=:bar\nWHERE id=123\nORDER BY id DESC\nLIMIT 1",
            $builder->makeQueryString($query)
    ...
    public function testMakeDeleteQuery()
    {
        ...
        $query = new Query();
        $query = $query->delete()->from('one')->where('foo=:foo');
        $this->assertEquals("DELETE FROM `one`\nWHERE foo=:foo", $builder->makeQueryString($query));
